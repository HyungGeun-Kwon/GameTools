...
namespace GameTools.Test.Utils
{
    public sealed class InMemoryRowVersionInterceptor : SaveChangesInterceptor
    {
        ...
        {
            if (ctx is null || !ctx.Database.IsInMemory()) return; foreach (var e in ctx.ChangeTracker.Entries())
            {
                // 엔티티에 RowVersion 속성이 있을 때만
                var prop = e.Properties.FirstOrDefault(p => p.Metadata.Name == "RowVersion");
                if (prop is null) continue;
                if (e.State == EntityState.Added && prop.CurrentValue is null) prop.CurrentValue = NewVersion();
                else if (e.State == EntityState.Modified) prop.CurrentValue = NewVersion(); // 업데이트 시 새 버전 시뮬레이션
            }
        }

...
namespace GameTools.Server.Test.DataBase.RarityTest
{
    public class CreateRarityTests
    {
        ...
        public async Task CreateRarity_SuccessInMemory()
        {
            ...
            var payload = new CreateRarityPayload("Common", "#000000");
            var rarityDto = await handler.Handle(new CreateRarityCommand(payload), CancellationToken.None);

            // 반환 Dto 기본 값 검증
            rarityDto.Id.Should().BeGreaterThanOrEqualTo(0);
            rarityDto.Grade.Should().Be("Common");
            rarityDto.ColorCode.Should().Be("#000000");
            rarityDto.RowVersion.Should().NotBeNullOrEmpty();
            ...
            saved.RowVersion.Should().BeEquivalentTo(rarityDto.RowVersion);
        }
        ...
        public async Task CreateRarity_SuccessSqlServer()
        {
            ...
            var rarityDto = await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Common", "#000000")), CancellationToken.None);
            ...
            saved.Grade.Should().Be("Common");
            saved.ColorCode.Should().Be("#000000");
            saved.RowVersion.Should().Equal(rarityDto.RowVersion);
        }
        ...
        public async Task CreateRarity_SuccessCreateAuditRarity()
        {
            ...
            var res = await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Common", "#000000")), CancellationToken.None);

            // 저장 확인
            var saved = await db.Set<Rarity>().AsNoTracking().SingleAsync(r => r.Id == res.Id);
            saved.RowVersion.Should().BeEquivalentTo(res.RowVersion);
        ...
        public async Task CreateRarity_FailWhenGradeNullOrWhitespace(string? grade)
        {
            ...
            var act = async () => await handler.Handle(new CreateRarityCommand(new CreateRarityPayload(grade, "#000000")), CancellationToken.None);
        ...
        public async Task CreateRarity_GradeMaxLengthBoundary()
        {
            ...
            var okRes = await handler.Handle(new CreateRarityCommand(new CreateRarityPayload(ok, "#000000")), CancellationToken.None);
            okRes.Grade.Length.Should().Be(max);

            var act = async () => await handler.Handle(new CreateRarityCommand(new CreateRarityPayload(tooLong, "#000000")), CancellationToken.None);
            await act.Should().ThrowAsync<Exception>();
        }
        ...
        public async Task CreateRarity_ColorCodeValidPasses(string color)
        {
            ...
            var res = await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("COLOR_OK", color)), CancellationToken.None);
        ...
        public async Task CreateRarity_ColorCodeInvalidFails(string color)
        {
            ...
            var act = async () => await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("COLOR_NG", color)), CancellationToken.None);
            await act.Should().ThrowAsync<Exception>();
        }
        ...
        public async Task CreateRarity_FailsWhenSameGrade()
        {
            ...
            await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Common", "#111111")), CancellationToken.None);

            var act = async () => await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Common", "#222222")), CancellationToken.None);
            await act.Should().ThrowAsync<Exception>();
        }
        ...
        public async Task CreateRarity_FailsWhenSameColorCode()
        {
            ...
            await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Common", "#000000")), CancellationToken.None);

            var act = async () => await handler.Handle(new CreateRarityCommand(new CreateRarityPayload("Rare", "#000000")), CancellationToken.None);
